...
describe("Authentication and User Management Integration Tests", () => {
  ...
  describe("Auth API", () => {
     ...
     */
    it("should allow Non-UCSD Request Access", async () => {
        const email = `non-ucsd-${Date.now()}@test.com`;
        createdUserEmails.push(email);

        const agent = request.agent(app);
        const response = await agent.post("/v1/api/auth/request-access").send({
            firstName: "Non", lastName: "UCSD", email, verificationCode: "ABC"
        });
