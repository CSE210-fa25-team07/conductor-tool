...
#### 5. Is_Enrolled
Student enrollments in courses. This table represents the relationship between a user and a course.

**Table**: `is_enrolled`
- `user_uuid` (PK, FK): Reference to users table
- `course_uuid` (PK, FK): Reference to courses table
- `enrollment_status`: "active", "dropped", or "completed"
...
- Track enrollment history and status
...
#### Student View
**Displays**: Course logistics, staff, and navigation buttons
**Tables Used**:
...
- `is_enrolled` (student's enrollment status)
...
**Query Pattern** (student's enrollment status):
```sql
...
#### Instructor View
**Displays**: Course logistics, class statistics, roster overview
**Tables Used**:
- `courses` (course info, logistics, links)
- `terms` (term information)
- `is_enrolled` (all student enrollments)
...
SELECT
    ...
    COUNT(CASE WHEN enrollment_status = 'completed' THEN 1 END) as completed_students
FROM is_enrolled
...
2. **Primary Keys**:
   ...
   - Composite keys for junction/relationship tables (`is_enrolled`, `course_staff`)
...
- Sensitive student data (contact info, enrollment status) should only be visible based on user role
